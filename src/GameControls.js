...
class GameControls extends Component {
	...
	getRandomBirdsong = function() {
		...
		let randomRecordingApiUrl = this.state.headToHeadSpecies ?
			`birdsong/head-to-head?thing=thing&${this.state.headToHeadSpecies.map(species => `species=${species.ScientificName}`).join(`&`)}` :
			`birdsong?level=${this.state.level}`
